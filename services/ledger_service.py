...
class LedgerService:
    ...
    1. 所有交易都是追加记录，不可修改
    2. 每条记录包含交易前后余额
    3. 使用行锁（SELECT ... FOR UPDATE）保证并发安全
    4. 事务边界：每次操作都在数据库事务内完成
    """
    ...
    def _acquire_account_lock(self, account_id: int) -> Account:
        ...
        使用 SELECT ... FOR UPDATE 锁定账户记录，防止并发修改。
    ...
    def append_credit(
        ...
        适用于：recharge（充值）、refund（退款）、adjust（调整-增加）
        ...
        Raises:
            UserNotFoundError: 用户不存在
            InvalidTransactionError: 交易类型不正确
        """
        # 验证交易类型
    ...
    def append_debit(
        ...
        适用于：pay（支付）、void（作废）、expire（过期）、adjust（调整-减少）
        ...
        try:
            ...
            if balance_before < amount_cents:
                ...
                raise InsufficientFundsError(
                    f"Account balance ({balance_before / 100.0:.2f} yuan) is insufficient "
    ...
    def append_debit_from_account(
        ...
        Args:
            ...
            related_txn_id: 关联交易ID（可选）
            remark: 备注（可选）
            operator_id: 操作员用户ID（可选，整数）
            booth_id: 摊位ID（可选，用于摊位管理系统）
            product_id: 商品ID（可选，用于摊位管理系统）
